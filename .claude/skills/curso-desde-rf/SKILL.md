...
**Antes de dar cualquier clase por terminada, corre la auditoría obligatoria de la Regla 6** (`references/reglas-de-clase.md` § Regla 6 — mismo estatus inquebrantable que la Regla fundamental, no una regla más entre ocho): enumerá cada párrafo de la clase que explica una funcionalidad o un comportamiento, y para cada uno, aislado de sus vecinos, verificá que tenga su propia representación pegada — cubriendo buena práctica, mala práctica y error cuando esos tres casos existan para ese concepto — y que el código tenga sus comentarios en la misma secuencia en que la prosa explicó cada cosa, nunca en un bloque único arriba. Una clase con algún "no" en esa lista no está lista, aunque el resto del contenido esté completo.
...
- **Todo el código generado (en clases y en `curso/app/`) lleva comentarios línea por línea, en la misma secuencia en que la prosa fue explicando cada cosa — nunca un bloque único `Entra:/Sale:` al principio, desconectado de las líneas reales.** La salida se marca donde sea más claro (misma línea del `return`, o al final del bloque). Ver `references/reglas-de-clase.md` § Regla 6 para el detalle y el ejemplo.
- **Regla 6 de `references/reglas-de-clase.md` (representación al cierre de cada párrafo) tiene el mismo estatus inquebrantable que la Regla fundamental — no es una regla más entre ocho.** Ninguna clase se da por terminada sin la auditoría obligatoria que esa regla exige (enumerar cada párrafo que explica funcionalidad y verificar, aislado de sus vecinos, que tiene su representación pegada — cubriendo buena práctica, mala práctica y error cuando esos casos existan).
