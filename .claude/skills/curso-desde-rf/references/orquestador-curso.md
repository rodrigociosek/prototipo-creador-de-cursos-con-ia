# Plantilla de `curso/00-orquestador.md`

Este archivo se crea en la Fase 0, junto a `curso/PROGRESS.md`, y no cambia después — es el mapa fijo del proceso, no el estado (eso es trabajo de `PROGRESS.md`). Existe para que **cualquiera que abra la carpeta `curso/` sola**, sin haber invocado el Skill ni leído la conversación, entienda de inmediato qué es cada archivo y qué proceso los generó.

**Qué NO hacer**: no edites este archivo para anotar avance, decisiones tomadas o el estado de una fase — eso pertenece a `curso/PROGRESS.md`. Solo se edita si la propia estructura de fases/archivos del Skill cambió, y aun así, mantenlo como un mapa, no como una bitácora.

Cópialo tal cual (ajustando solo lo que dependa del proyecto concreto, como rutas si difieren):
...
- `guia_aprendizaje.md` (qué información investigar y recolectar para enseñar cada concepto completo — no una plantilla de secciones de clase)

**Archivos del motor (parte de la instalación del Skill en `.claude/skills/curso-desde-rf/references/`, no del proyecto):**
- `checklists-auditoria.md`, `estado-progreso.md`, `simulaciones.md`, `orquestador-curso.md`, `formato-indice.md`, `formato-dependencias.md`, `formato-prioridad-rf.md`, `formato-conceptos-ensenados.md`, `formato-tareas.md`, `reglas-de-clase.md`
...
| 4 | Desglosar cada RF en tareas concretas, el qué — Nivel 2, en orden de prioridad, cada una responsabilidad única (ver `formato-tareas.md`) | `curso/rf.md`, `curso/prioridad-rf.md`, `references/formato-tareas.md` | `curso/tareas.md` |
| 4.1 | Auditar huecos de tareas y tareas todavía bundled (checkpoint) | `curso/tareas.md`, `curso/rf.md`, `references/formato-tareas.md` | `curso/auditoria-tareas.md` |
| 5 | Especificar cada tarea hasta el cómo, incluyendo qué necesita — Nivel 3 | `curso/tareas.md`, `references/formato-tareas.md` | actualiza `curso/tareas.md` |
...
| 8 | Generar las clases de conocimiento y de construcción, una por una, siguiendo `curso/indice.md` | `curso/indice.md` (qué toca y en qué orden — nunca la única fuente), **`curso/tareas.md`** (especificación Nivel 3 completa de cada T-XX que la clase construye: Dónde/Implica/Necesita, Modelo de datos, Decisiones técnicas transversales), **`curso/stack.md`/`curso/auditoria-stack.md`** (versión exacta confirmada de cada tecnología que la clase use), `curso/PROGRESS.md`, `curso/conceptos-enseñados.md`, `curso/dependencias.md`, `references/reglas-de-clase.md`, `guia_aprendizaje.md`, estado actual de `curso/app/` + investigación real por tema | `curso/clases/clase-NN-slug.md`, `curso/app/`, actualiza `curso/simulaciones.md`, `curso/conceptos-enseñados.md` y `curso/PROGRESS.md` |
...
├── guia_aprendizaje.md              ← entrada: qué investigar para enseñar completo
